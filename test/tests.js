...
QUnit.module("getSectionCombinations", {
  ...
  afterEach: function() {
    ...
    QUnit.test("Filter section combinations by crn-specific selections", function(assert) {
        // Ignore filters, don't filter anything out
        mscSchedulizer.applyFiltersToSection = sandbox.stub().returns(false);
        //TODO-KL
        assert.equal(false,false,"false is false");
    });
    QUnit.test("crn-specific selection should be in the result of all combinations", function(assert) {
        // Ignore filters, don't filter anything out
        mscSchedulizer.applyFiltersToSection = sandbox.stub().returns(false);
        mscSchedulizer.classes_selected = [
            {
